...
climatology_areas_origem_regional <-
  ...
  left_join(., prec_df, by = c('DATE', 'ID')) %>% mutate(DATE = str_replace(DATE, "X", ""),
                                                         DATE = as_date(DATE),
                                                         ID = as.character(ID))
...
climatology_areas_origem_regional <-
  dplyr::left_join(climatology_areas_origem_regional,
                   geo_loc_areas_origem@data,
                   by = "ID") %>%
  dplyr::select(REGIAO_ORIGEM, UF, everything(),-ID)

write_csv(
  climatology_areas_origem_regional,
  "data/coffee-project/climatology_or_regional_yearly.csv"
)
...
climatology_areas_origem_municipal <-
  ...
  left_join(., prec_df, by = c('NOME', 'UF', 'REGIAO_ORIGEM', 'DATE')) %>% mutate(DATE = str_replace(DATE, "X", ""),
                                                                                  DATE = as_date(DATE)) %>%
...
write_csv(
  climatology_areas_origem_municipal,
  "data/coffee-project/climatology_or_municipal_yearly.csv"
)
...
climatology_areas_origem_regional <-
  climatology_areas_origem_regional %>% mutate(
    TMIN = na_if(TMIN, NaN),
    TMAX = na_if(TMAX, NaN),
    PREC = na_if(PREC, NaN)
  )
...
write_parquet(
  climatology_areas_origem_regional,
  "data/coffee-project/climatology_or_regional_daily.parquet"
)
